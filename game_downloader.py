...
def round_down_to_nearest_half_hour(dt: datetime) -> datetime:
    ...
    rounded_time = dt - timedelta(minutes=minutes_to_subtract, seconds=dt.second, microseconds=dt.microsecond)
    return rounded_time
...
def download_game(
    ...
    for url_num, pure_url in enumerate(pure_urls):
        download.download_file(pure_url, game_folder.joinpath(f"part{url_num}.mp4"))
