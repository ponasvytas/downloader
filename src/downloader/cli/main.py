...
from ..utils.helpers import get_video_directory, validate_environment_variables
...
from ..core.concatenator import concatenate_game

# Sample list of items to search from
rink_names = RINK_NAMES.values()
rink_codes = RINK_NAMES.keys()
teams = ["12U Storm North", "12U Storm Select", "...other"]
...
def download():
    ...
    selected_rink_code = [
        code for code, name in RINK_NAMES.items() if name == rink_name
    ][0]

    start_date_input = prompt(
        "Game Start Date: ", default=datetime.now().date().strftime("%Y-%m-%d")
    )
    start_date = datetime.strptime(start_date_input, "%Y-%m-%d").date()

    start_time_input = prompt("Game Start Time [HH:MM]: ")
    start_time = datetime.strptime(start_time_input, "%H:%M").time()

    game_length = float(prompt("Game Length (in hours): ", default="1.00"))

    use_pano_mode = (
        prompt("Use Pano Mode? (True/False): ", default="False").strip().lower()
        == "true"
    )
    ...
    # Get the project root directory (where video folder should be)
    project_root = Path(__file__).parent.parent.parent.parent
    root_path = project_root.joinpath("video", selected_team)
...
def concat():
    ...
    # Get the project root directory (where video folder should be)
    project_root = Path(__file__).parent.parent.parent.parent
    game_folder = project_root.joinpath("video", selected_team)
    ...
    concatenate_game(
        selected_game,
        trim_start_time=trim_at_start,
        trim_end_time=trim_at_end,
        delete_parts=False,
    )
