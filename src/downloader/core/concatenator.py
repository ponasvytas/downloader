...
def read_video_details(path_to_video: Path) -> VideoDetails:
    ...
    return VideoDetails(
        frame_rate=frame_rate, height=height, width=width, length=length
    )
...
def trim_video_files(directory: Path, trim_start_time, trim_end_time):
    ...
    if trim_start_time and trim_start_time != "00:00:00":
        ...
        ffmpeg.input(mp4_files[0], ss=trim_start_time).output(
            f"{mp4_files[0].stem}_trim.mp4", c="copy"
        ).run()
        # remove input file
        mp4_files[0].unlink()
    ...
    if trim_end_time and trim_end_time != "00:00:00":
        ...
        ffmpeg.input(mp4_files[-1], t=trim_end_time).output(
            f"{mp4_files[-1].stem}_trim.mp4", c="copy"
        ).run()
        # remove input file
        mp4_files[-1].unlink()
...
def concatenate_game(
    game_folder: Path,
    trim_start_time: Optional[
        str
    ] = None,  # timestamp in HH:MM:SS format of the first part file
    trim_end_time: Optional[
        str
    ] = None,  # timestamp in HH:MM:SS format of the last part file
    delete_parts: bool = False,
):
    trim_video_files(game_folder, trim_start_time, trim_end_time)

    game_name = game_folder.name
    video_path = concatenate_files(
        game_folder, game_name=game_name, delete_parts=delete_parts
    )
